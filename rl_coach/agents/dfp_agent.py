...
class DFPAgent(Agent):
    ...
    def learn_from_batch(self, batch):
        ...
        network_inputs["goal"] = np.repeat(np.expand_dims(self.current_goal, 0), batch.size, axis=0)
    ...
    def choose_action(self, curr_state):
        if self.exploration_policy.requires_action_values():
            ...
            for action_idx in range(len(self.spaces.action.actions)):
                ...
                action_measurements = np.reshape(
                    action_measurements,
                    (self.ap.algorithm.num_predicted_steps_ahead, self.spaces.state["measurements"].shape[0]),
                )
                future_steps_values = np.dot(action_measurements, self.current_goal)
                action_values[action_idx] = np.dot(
                    future_steps_values[-num_steps_used_for_objective:], self.ap.algorithm.future_measurements_weights
                )
        else:
            action_values = None
    ...
    def set_environment_parameters(self, spaces: SpacesDefinition):
        ...
        self.spaces.goal = VectorObservationSpace(
            ...
            measurements_names=self.spaces.state["measurements"].measurements_names,
        )
        ...
        self.target_measurements_scale_factors = np.array(
            [
                self.ap.algorithm.scale_measurements_targets[measurement_name]
                for measurement_name in self.spaces.state["measurements"].measurements_names
            ]
        )
    ...
    def handle_episode_ended(self):
        ...
        if self.phase in [RunPhase.TRAIN, RunPhase.HEATUP] and last_episode:
            self._update_measurements_targets(last_episode, self.ap.algorithm.num_predicted_steps_ahead)
            ...
            for step in range(num_steps):
                ...
                if offset_idx >= episode.length():
                    if self.ap.algorithm.handling_targets_after_episode_end == HandlingTargetsAfterEpisodeEnd.NAN:
                        ...
                        self.ap.algorithm.handling_targets_after_episode_end == HandlingTargetsAfterEpisodeEnd.LastStep
                    ):
                        offset_idx = -1

                transition.info["future_measurements"][step] = self.target_measurements_scale_factors * (
